...
//compiling functionality for game
export default class Board extends React.Component {
  state = {
    //sets the initial state of our board of the game
    boardSettings: this.initBoardSettings(
      this.props.height,
      this.props.width,
      this.props.dogs
    ),
    //setting game status when won or not
    gameWon: false,
    //number of dogs
  ...
  getDogs(data) {
    ...
    let squareArray = [];
    //using map and array to dispaly the dog
    data.map((datarow) => {
    ...
    //returns our dogs in in the squares
    return squareArray;
  }
  ...
  handleCellClick(x, y) {
    ...
    if (this.state.boardSettings[x][y].isDog) {
      this.revealBoard();
      swal(
        "Oopsie, we opened a door and a dog chased away all the kittens! It seems that in our defeat, the dog left us a present. What do you want to do? 🙀",
        {
          title: "GAME OVER!",
          buttons: {
            quit: {
    ...
    if (this.getHidden(updatedData).length === this.props.dogs) {
      ...
      this.revealBoard();
      swal(
        "Yay, we found all the kittens! Now Ms. Crumblebottom can't yell at me. Here's a little thank you.",
        {
          title: "GAME WON!",
          buttons: {
            quit: {
  ...
  render() {
    return (
      <div className="board">{this.renderBoard(this.state.boardSettings)}</div>
    );
  }
}
